...
describe('Cars', function () {
  ...
  describe('Create Ad', function () {
    ...
    it('should return error 400 if there is no image', function (done) {
      ...
      _chai["default"].request(_index["default"]).post(adUrl).set('x-auth', token).send(data).end(function (err, res) {
    ...
    it('should return error 401 if token is not provided', function (done) {
      ...
      _chai["default"].request(_index["default"]).post(adUrl).send(data).end(function (err, res) {
  ...
  describe('view available cars by manufacturer', function () {
    ...
    it('should return all unsold cars by a manufacturer', function (done) {
      ...
      _chai["default"].request(_index["default"]).get("/api/v1/car/manufacturer/".concat(manufacturers[0])).end(function (err, res) {
    ...
    it('should return a custom error if no vehicle is found for the manufacturer', function (done) {
      ...
      _chai["default"].request(_index["default"]).get('/api/v1/car/manufacturer/Chidi').end(function (err, res) {
  ...
  describe('view available cars by body type', function () {
    ...
    it('should return all unsold cars by body type', function (done) {
      ...
      _chai["default"].request(_index["default"]).get("/api/v1/car/bodytype/".concat(bodyType[1])).end(function (err, res) {
        expect(res.status).to.eq(200);
        expect(res.body).to.have.property('data').to.be.an('Array');
        done();
      });
    ...
    it('should return error 404 if cars of given body type are not found', function (done) {
      ...
      _chai["default"].request(_index["default"]).get("/api/v1/car/bodytype/".concat(bodyType[2])).end(function (err, res) {
        expect(res.status).to.eq(404);
  ...
  describe('view available cars by state', function () {
    ...
    it('should return all available cars by state -used', function (done) {
      ...
      _chai["default"].request(_index["default"]).get("/api/v1/car/state/".concat(state[0])).end(function (err, res) {
        expect(res.status).to.eq(200);
        expect(res.body).to.have.property('data').to.be.an('ARRAY');
    ...
    it('should return all available cars by state -new', function (done) {
      ...
      _chai["default"].request(_index["default"]).get("/api/v1/car/state/".concat(state[1])).end(function (err, res) {
        expect(res.status).to.eq(200);
        expect(res.body).to.have.property('data').to.be.an('ARRAY');
    ...
    it('should return error 404 if cars are not found for selected state -old', function (done) {
      ...
      _chai["default"].request(_index["default"]).get('/api/v1/car/state/old').end(function (err, res) {
        expect(res.status).to.eq(404);
  ...
  describe('view all available cars', function () {
    it('should return all unsold cars', function (done) {
      ...
      _chai["default"].request(_index["default"]).get('/api/v1/cars/status/available').end(function (err, res) {
        expect(res.status).to.eq(200);
        expect(res.body).to.have.property('data').to.be.an('ARRAY');
        done();
      });
  ...
  describe('Get ad by id', function () {
    it('should return a single ad details', function (done) {
      ...
      _chai["default"].request(_index["default"]).get("/api/v1/car/".concat(id)).end(function (err, res) {
        expect(res.status).to.eq(200);
    ...
    it('should return error 400 with custom message if supplied id is not valid', function (done) {
      ...
      _chai["default"].request(_index["default"]).get('/api/v1/car/12345678901').end(function (err, res) {
        expect(res.status).to.eq(400);
    ...
    it('should return error 404 with custom message if ad is not found', function (done) {
      ...
      _chai["default"].request(_index["default"]).get('/api/v1/car/9293837414384').end(function (err, res) {
        expect(res.status).to.eq(404);
  ...
  describe('Seller update ad price', function () {
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 4:
              ...
              _carsData["default"][0].owner = user.id;
              reqData = {
                id: _carsData["default"][0].id,
                price: 2400000,
                description: 'This is to add further description'
              };
              _context.next = 9;
              return _chai["default"].request(_index["default"]).patch("/api/v1/car/".concat(reqData.adId)).set('x-auth', token).send(reqData);
    ...
    it('should return error 404 if ad is not found', function () {
      ...
      _chai["default"].request(_index["default"]).patch("/api/v1/car/".concat(reqData.adId)).set('x-auth', token).send(reqData).end(function (err, res) {
        expect(res.status).to.eq(404);
    ...
    it('should return error 401 if another user attempts update an ad', function () {
      ...
      user.isAdmin = false;
      var token = (0, _generateToken["default"])(user.id, user.isAdmin);
      var price = _carsData["default"][0].price - 1000000;
    ...
    it('should return error 401 if user is not logged in', function () {
      ...
      var reqData = {
        id: _carsData["default"][0].id,
        price: _carsData["default"][0].price - 100,
        description: 'This is to add further description'
      };

      _chai["default"].request(_index["default"]).patch("/api/v1/car/".concat(reqData.adId)).send(reqData).end(function (err, res) {
        expect(res.status).to.eq(401);
        expect(res.body.message).to.eq('No authorization token provided');
      });
    });
  ...
  describe('User can view single ad', function () {
    ...
    it('should return error 404 if ad is not found', function (done) {
      carsArray();
      var id = _carsData["default"][0].id + 1;
    ...
    it('should return error 400 if invalid ad id is supplied', function (done) {
      ...
      _chai["default"].request(_index["default"]).get('/api/v1/car/155873165645').end(function (err, res) {
        expect(res.status).to.eq(400);
        expect(res.body.message).to.eq('Invalid ad id');
        done();
      });
  ...
  describe('Get ads within a price range', function () {
    it('should return an array of ads within a price range', function (done) {
      ...
      _chai["default"].request(_index["default"]).get('/api/v1/car/price/?min=5000000&max=8000000').end(function (err, res) {
        expect(res.status).to.eq(200);
    ...
    it('Minimum should default to 0 if not supplied', function (done) {
      ...
      _chai["default"].request(_index["default"]).get('/api/v1/car/price/?max=8000000').end(function (err, res) {
        expect(res.status).to.eq(200);
        expect(res.body.data).to.be.an('ARRAY');
        done();
      });
    ...
    it('Maximum should default to 24000000 if not supplied', function (done) {
      ...
      _chai["default"].request(_index["default"]).get('/api/v1/car/price/?min=2000000').end(function (err, res) {
        expect(res.status).to.eq(200);
        expect(res.body.data).to.be.an('ARRAY');
        done();
      });
    ...
    it('Should return error 404 if no ads are found in the given range', function (done) {
      ...
      _chai["default"].request(_index["default"]).get('/api/v1/car/price/?min=12000000&max=24000000').end(function (err, res) {
        expect(res.status).to.eq(404);
  ...
  describe('admin view all ads', function () {
    it('should return all ads', function (done) {
      ...
      _chai["default"].request(_index["default"]).get('/api/v1/car').set('x-auth', token).end(function (err, res) {
        expect(res.status).to.eq(200);
        expect(res.body.data).to.be.an('Array');
  ...
  describe('Admin can delete a posted ad', function () {
    it('should delete a posted ad', function (done) {
      ...
      _chai["default"].request(_index["default"])["delete"]("/api/v1/car/".concat(_carsData["default"][0].id)).set('x-auth', token).end(function (err, res) {
        expect(res.status).to.eq(200);
    ...
    it('should return error 401 if user is not admin or not logged in', function (done) {
      ...
      _chai["default"].request(_index["default"])["delete"]("/api/v1/car/".concat(_carsData["default"][0].id)).end(function (err, res) {
        expect(res.status).to.eq(401);
        expect(res.body.message).to.eq('No authorization token provided');
        done();
      });
    ...
    it('should return error 404 if wrong ad id is given', function (done) {
      ...
      _chai["default"].request(_index["default"])["delete"]("/api/v1/car/".concat(id)).set('x-auth', token).end(function (err, res) {
        expect(res.status).to.eq(404);
